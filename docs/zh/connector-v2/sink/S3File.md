...
| fs.s3a.aws.credentials.provider       | string  | 是       | software.amazon.awssdk.auth.credentials.InstanceProfileCredentialsProvider | 认证 s3a 的方式。目前仅支持 `org.apache.hadoop.fs.s3a.SimpleAWSCredentialsProvider` 和 `software.amazon.awssdk.auth.credentials.InstanceProfileCredentialsProvider` 和 `software.amazon.awssdk.auth.credentials.DefaultCredentialsProvider`。 |
